...
def browser_type_launch_args(pytestconfig: Any) -> Dict:
    ...
    # if maximized_option:
    #     launch_options["start_maximized"] = True
        ...
        for page in pages:
            ...
            try:
                ...
                allure.attach.file(file_path,
                                   name=f"{request.node.name}-{human_readable_status}-{index}",
...
def pytest_addoption(parser: Any) -> None:
    ...
    group.addoption(
        ...
        default=0,
        type=int,
        help="Run cases with slow mo",
    )
    group.addoption(
        "--device", default=None, action="store", help="Device to be emulated."
    )
    # print("------------", get_application_config().get('reports_path').get('results'))
    group.addoption(
        "--output",
        default=f"{get_project_path()}/{get_application_config().get('reports_path').get('results')}/{today}/{now}",
        help="Directory for artifacts produced by cases, defaults to test-results.",
    )
    group.addoption(
        "--tracing",
        default="off",
